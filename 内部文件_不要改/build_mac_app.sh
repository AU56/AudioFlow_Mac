...
PAYLOAD_DIR="AudioFlow_Studio_macOS_v${APP_VERSION}"
...
if [ -f "README_Mac.txt" ]; then
  cp "README_Mac.txt" "$PAYLOAD_DIR/README_Mac.txt"
...
if [ -f "Open_AudioFlow.command" ]; then
  cp "Open_AudioFlow.command" "$PAYLOAD_DIR/Open_AudioFlow.command"
  chmod +x "$PAYLOAD_DIR/Open_AudioFlow.command"
fi

xattr -cr "$PAYLOAD_DIR" || true
ditto -c -k --sequesterRsrc --keepParent "$PAYLOAD_DIR" "$ZIP_NAME"
